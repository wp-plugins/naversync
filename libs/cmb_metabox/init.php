...
class cmb_Meta_Box {
	...
	function add() {
		...
		foreach ($this->_meta_box['pages'] as $page) {
			if (apply_filters('cmb_show_on', true, $this->_meta_box))
				add_meta_box($this->_meta_box['id'], $this->_meta_box['title'], array(&$this,
				                                                                      'show'
			...
			switch ($field['type']) {
				...
				case 'file':
					...
					if (!empty($meta)) {
						...
						$valid    = (array)apply_filters('cmb_valid_img_types', array('jpg',
						                                                              'jpeg',
						                                                              'png',
						                                                              'gif',
						                                                              'ico',
	...
	function save($post_id, $post) {
		...
		foreach ($this->_meta_box['fields'] as $field) {
			...
			if ($type_comp == true && in_array($field['type'], array('taxonomy_select',
			                                                         'taxonomy_radio',
			                                                         'taxonomy_multicheck'
				))
			) {
...
function cmb_scripts($hook) {
	...
	if ($hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php') {
		...
		wp_localize_script('cmb-scripts', 'cmb_ajax_data', array('ajax_nonce' => wp_create_nonce('ajax_nonce'),
		                                                         'post_id' => get_the_ID()
		));
